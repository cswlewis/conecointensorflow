...
def gather_and_save_data():
    ...
    for i in range(lookbacks+minutesToDecideOver+75, df[columns_to_normalize].shape[0]-lookbacks-minutesToDecideOver-75):
        df3 = df[columns_to_normalize].iloc[i-lookbacks-75:i].values

        if(i % 200 == 0 or i ==lookbacks+minutesToDecideOver+75):
            print(i/dataLength*100)
            
        X[i-1] = (df3[75:] - df3.mean()) / df3.std()
    ...
    np.savetxt('X_data_full50-15s.csv', X.reshape((X.shape[0], -1)), delimiter=',')
    np.savetxt('y_data_full50-15s.csv', y, delimiter=',')
...
def main():
    ...
    loaded_X = np.loadtxt('X_data_full50-15s.csv', delimiter=',').reshape((-1, lookbacks, numberOfIndicators))
    loaded_y = np.loadtxt('y_data_full50-15s.csv', delimiter=',')
    ...
    with tf.device("/device:GPU:0"):    
        ...
        # model.load_weights(path_checkpoint)
        model.save('model-gpu-rnn-1m-mse-15s')
